...
namespace VLang.Frontends
{
    public class DefaultFrontend : IFrontend
    {
        ...
        public ASTNode Parse(string script)
        {
            ...
            {
                root.Insert(0, new VariableDeclaration("string", new string[0], elem.Key, new Expression(new List<IASTElement>
                {
        ...
        public IASTElement ToRPN(string exp)
        {
            ...
            // this replaces functions to stub - it makes possible to rpn those
            ...
            // functions too but without args aa()
            funccc = Regex.Match(exp, "([A-z0-9]+)(\\()(\\))");
            while(funccc.Success)
            {
                exp = HashFunction(exp, funccc, ref stubs);
            ...
            funccc = Regex.Match(exp, "[0-9]+(\\.)[0-9]+");
            while(funccc.Success)
            {
            ...
            foreach(string atm in elems)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            while(rpns.Count > 0 && operators[rpns.Peek()] >= operators[atm])
                                output.Add(rpns.Pop());
                            rpns.Push(atm);
                        }
                    }
            ...
            for(int i = 0; i < output.Count; i++)
            {
                ...
                output[i] = output[i].Replace('\x7', '.').Trim();
            }
        ...
        private IASTElement CreateExpression(string[] rpn)
        {
            ...
            foreach(string chane in rpn)
            {
                ...
                switch(GetElementType(elementNoSpaces))
                {
                    ...
                    {
                        string[] split2 = GammaSplitOne(withSpaces, '=');
                        string name = split2[0];
                        string exp = split2[1];
                        if(name.Contains(" "))
                        {
                            var split = GammaSplit(name, ' ').ToList();
                            string varname = split.Last();
                            split.RemoveAt(split.Count - 1);
                            string type = split.Last();
                            split.RemoveAt(split.Count - 1);
                            list.Add(new VariableDeclaration(type, split.ToArray(), varname, ToRPN(exp)));
                        }
                        else
                        {
                    ...
                    case ElementType.Variable:
                    {
                        ...
                        else
                        {
                            if(chane.Contains(" "))
                            {
                                var split = GammaSplit(chane, ' ').ToList();
                                string varname = split.Last();
                                split.RemoveAt(split.Count - 1);
                                string type = split.Last();
                                split.RemoveAt(split.Count - 1);
                                list.Add(new VariableDeclaration(type, split.ToArray(), varname, new Value(null)));
                            }
                            else
                            {
                    ...
                    {
                        string[] arguments = TrimBraces(CutExpression(chane)).Split(',').Select(a => a.Trim()).ToArray();
                        string fullname = FlattenKeepBraces(chane).Split('(')[0];
                        string fullnameNoSpaces = FlattenKeepBraces(elementNoSpaces).Split('(')[0];
                        var split = GammaSplit(fullname, ' ').ToList();
                        string name = split.Last();
                        split.RemoveAt(split.Count - 1);
                        int groupId = int.Parse(Flatten(elementNoSpaces).Trim().Replace(fullnameNoSpaces + "_reserved_codegroup_", ""));
                        list.Add(new FunctionDefinition(name, split.ToArray(), arguments, groupId));
                    }
                    break;
        ...
        private ElementType GetElementType(String element)
        {
            ...
            else if(element.Trim().StartsWith("new"))
                return ElementType.New;
            if(operators.Keys.Contains(element))
                return ElementType.Operator;
            if(Regex.IsMatch(element, @"[^0-7]") == false && element.StartsWith("0"))
                return ElementType.Octan;
            else if(Regex.IsMatch(element, @"[^0-9]") == false)
                return ElementType.Integer;
            else if(Regex.IsMatch(element, @"[^0-9.]") == false)
                return ElementType.Double;
            else if(Regex.IsMatch(element, @"[^0-9a-fA-Fx]") == false && element.StartsWith("0x"))
                return ElementType.Hex;
            else if(Regex.IsMatch(element, @".+?(`).+?") == true && Regex.IsMatch(element, @"[\(\)=]") == false)
                return ElementType.Cast;
            else if(Regex.IsMatch(element, @"[\(\)=]") == false)
                return ElementType.Variable;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^if\(\).+else.+$") == true)
                return ElementType.IfWithElse;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^if\(\).+$") == true)
                return ElementType.If;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^while\(\).+$") == true)
                return ElementType.While;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^for\(\).+$") == true)
                return ElementType.For;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^foreach\(\).+$") == true)
                return ElementType.Foreach;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^map\(\).+$") == true)
                return ElementType.Map;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^filter\(\).+$") == true)
                return ElementType.Filter;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"^loop\(\).+$") == true)
                return ElementType.Loop;
            else if(Regex.IsMatch(element.Replace(" ", ""), @".+\(\)_reserved_codegroup_([0-9]+)$") == true)
                return ElementType.NamedFunction;
            else if(Regex.IsMatch(element.Replace(" ", ""), @"\)_reserved_codegroup_([0-9]+)$") == true)
                return ElementType.AnonymousFunction;
        ...
        string HashFunction(string exp, Match match, ref Dictionary<String, String> stubs)
        {
            ...
            if(CountBraces(exp) != 0)
                throw new Exception("Syntax parsing error");
            return exp;
        }
